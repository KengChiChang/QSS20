...
def should_display(entry, last_lecture):
    if "entries" in entry:  # This is a category
        display = False
        for child_entry in entry["entries"]:
            if should_display(child_entry, last_lecture):
                display = True
        entry["should_display"] = display
        return display
    else:  # A regular command
        display = False
        for lec in entry["lectures"]:
            if lec <= last_lecture:
                display = True
        entry["should_display"] = display
        return display
...
def set_command_reference_visibility(env):
    """
    Adds a should_display property to the command config to hide ones we
    haven't covered yet
    """
    last_lecture = env.variables["commands"]["publish"]
    commands = env.variables["commands"]["commands"]
    for entry in commands:
        should_display(entry, last_lecture)
